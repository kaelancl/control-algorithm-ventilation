...
load('patient_data/20_breaths_pat_12_1511_1903.mat')
...
while counter < num_breaths +1
   ...
   for i = 1:19
        ...
        if volume(1) < 500
            ...
            % Peaks + lengths
            [peaks_array(counter,1),peak_time_array(counter,1)] = min(pes);
            if length(find(diff(phase) == 32)) == 1
                ...
                lb = ones(M,1)*-100;
                %lb = zeros(M,1);
                coeff_bs = lsqlin(phi_2',pes,[],[],[],[],lb,[]);
    
                pes_est = coeff_bs'*phi_2;
...
pes_model = coeff_med_norm*phi_2;
